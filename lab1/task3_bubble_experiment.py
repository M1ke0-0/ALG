...
Эксперимент с пузырьковой сортировкой

- Реализация bubble_sort
- Замер времени для n = [100, 200, 400, 800, 1600, 3200]
- Каждый эксперимент повторяется 5 раз, берётся среднее
- График 1: время от n  (парабола → O(n²))
- График 2: время от n² (прямая  → подтверждение O(n²))
"""
...
def measure_time(sort_func, arr, repeats=5):
    """Возвращает среднее время выполнения за repeats повторов (в секундах)"""
    times = []
